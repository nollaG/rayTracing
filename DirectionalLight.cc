...
DirectionalLight::DirectionalLight(const GVector3& dir):m_Direction(dir){}
void DirectionalLight::setDirection(const GVector3& dir) {
  m_Direction=dir;
}
GVector3 DirectionalLight::getDirection() const {
  return m_Direction;
}

GVector3 DirectionalLight::calAmbient(const GVector3& material_Ka) const {
...
GVector3 DirectionalLight::calDiffuse(const GVector3& material_Kd,const GVector3& normal,const GVector3& rayDirection) const {
...
GVector3 DirectionalLight::calSpecular(const GVector3& material_Ks,const GVector3& normal,const GVector3& rayDirection,const GVector3& cameraDirection,double material_shinness) const {
...
GVector3 DirectionalLight::calColor(const GObject* obj,const GVector3& point,const GVector3& cameraPosition) const {
  GVector3 color(0.0f,0.0f,0.0f);
  GVector3 normal=obj->getNormal(point);
  normal.normalize();
  GVector3 ambient = calAmbient(obj->getKa());
  GVector3 rayDirection = getDirection() * -1.0f;
...
GVector3 DirectionalLight::getLightDirection(const GVector3& point) const {
  return getDirection();
}
